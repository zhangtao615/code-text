...
export default {
  ...
  mounted() {
    window.onscroll = () => {
      ...
      if(clientHeight + scrollTop >= scrollHeight - 30 && this.isloading && this.movieList.length !=25){
        this.liatStart += 10;
        this.getData()
      }
    }
...
created(){
  ...
  methods: {
    getData(){
      ...
      let url = `https://m.douban.com/rexxar/api/v2/subject_collection/movie_showing/items?star=${this.movueStart}&count=10`
      this.isLoading = false;
      axios.get(proxy+url)
      .then((res) => {
        this.movieList = this.movieList.concat(res.data.subject_collection_items);
        this.isLoading = true;
...
<style scoped>
  ...
  .movieList-right .movie-actors,.movie-year{
    font-size: 12px;
  }
  .loading{
